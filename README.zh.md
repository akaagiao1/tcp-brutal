...
添加菜单默认目标为 `0.0.0.0/0`，目标提示处直接回车即可使用，随后填写带宽。它会让 VPS 的全部 IPv4 TCP 目标使用并共享该 Brutal 带宽，适合整台 VPS 仅供自己使用的情况。同一规则重复添加会更新带宽，不同网段可以保留多条规则。删除会同时移除当前规则和保存的配置，防止重启后恢复旧规则。删除规则不会强制断开已有 TCP 连接；请重新连接代理。

`0.0.0.0/0` 在路由层使用 `0.0.0.0/1` 和 `128.0.0.0/1` 两条受管路由实现，从而覆盖全部 IPv4，同时保留系统原有默认路由及其参数。`brutalctl list` 中 `/0` 应显示 `ROUTE yes`；若显示 `ROUTE no`，说明规则尚未真正接管连接，需要更新工具后重新添加。
...
菜单同样支持 CIDR。输入裸 IP `223.80.170.224` 或 `223.80.170.224/24`，都会自动规范化为 `223.80.170.0/24`。如只需匹配一个地址，请明确输入 `223.80.170.224/32`。支持 `/0` 至 `/32`；`/0` 会覆盖所有 IPv4 目标，应明确了解范围后再使用。运营商换 IP 时不保证仍落在同一 `/24`。

交互菜单默认使用 `/0`；直接命令 `bash install.sh add 223.80.170.224 50` 中的裸 IP 仍按 `/24` 规范化。需要删除默认规则时执行 `bash install.sh delete 0.0.0.0/0`。

规则重叠时最长前缀优先：旧 `/32` 仍会优先于新 `/24`；如要改用网段规则，应明确删除不再需要的旧 `/32`。添加网段不会自动删除已有单 IP 规则。为防止升级后意外扩大范围，旧版配置文件中已经保存的裸 IP 仍按 `/32` 读取；新版保存时始终写出完整 CIDR。
